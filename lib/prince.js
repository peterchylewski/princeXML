...
function Prince(options, fn) {
	...
	if (options instanceof PrinceOptions) {
		this.options = options;
	} else {
		this.options = PrinceOptions(options);
	}
...
PrinceOptions.prototype = {
  ...
  get input() { 
    if (!this._input ) {
		throw new Error("Missing input files to be generated");
	}
    return this._input;
  },
  ...
  get binary() { 
    ...
    if (!bin) {
		throw new Error("Prince not find in your PATH");
	} else {
		console.log('prince binary found!');
	}
    return bin;
  },
